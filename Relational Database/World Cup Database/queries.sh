...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE round='Final' AND year=2018 ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games ON teams.team_id = games.winner_id LEFT JOIN games AS ogames ON teams.team_id = ogames.opponent_id WHERE name LIKE 'Co%' ORDER BY name")"
